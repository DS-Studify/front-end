    ...
    Box(
        ...
        Column(
            ...
            OutlinedTextField(
                ...
                onValueChange = updateCurrentPassword,
                label = {
                    Text(
                        text = stringResource(id = StudifyString.mypage_edit_password_label),
                        color = StudifyColors.G03,
                        style = Typography.bodyMedium
                    )
                },
                isError = uiState.currentPasswordErrorRes != null,
                singleLine = true,
                modifier = Modifier.fillMaxWidth(),
                keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Password),
                visualTransformation = PasswordVisualTransformation(),
                colors = OutlinedTextFieldDefaults.colors(
                    focusedBorderColor = if (uiState.currentPasswordErrorRes != null) StudifyColors.RED else StudifyColors.PK03,
                    unfocusedBorderColor = if (uiState.currentPasswordErrorRes != null) StudifyColors.RED else StudifyColors.G03,
                    cursorColor = StudifyColors.PK03
                )
            )

            if (uiState.currentPasswordErrorRes != null) {
                Text(
                    text = stringResource(id = uiState.currentPasswordErrorRes),
